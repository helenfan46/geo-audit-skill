...
Give the AI agent a firm name and website URL. The agent runs in one of two modes:

**Full audit (default, ~15–20 minutes):**
1. Research the firm and auto-identify **5 competitors**
...
4. Spawn **15 isolated sub-agents** — each answers a single prompt with zero research context — and record which firms get mentioned
...
**Demo (faster, ~5 minutes):**
1. Research the firm and auto-identify **3 competitors**
2. Audit what content each competitor publishes
3. Generate **5 prompts** from competitor website content + general web search (skip dedicated Reddit and Google PAA sub-agents)
4. Spawn **5 isolated sub-agents** to answer each prompt cold
5. Produce the same narrative-first report structure, flagged with a "Demo run" note in the disclaimer

**Agent behavior at start (MANDATORY):** Before doing any research, tell the user:

> *"This will take ~15–20 minutes for the full audit. I'd recommend starting with the Demo version (~5 min) to preview the output before committing to the full run. Say 'demo' for the Demo version, or 'full' to proceed with the comprehensive audit. If you don't specify, I'll run the Full audit."*

If the user picks Demo, run in Demo mode. If the user picks Full or doesn't respond with a preference, run in Full mode.
...
**For Demo runs, append a second italic line to the disclaimer:** *This is a 5-prompt Demo baseline. The 15-prompt Full audit provides higher confidence — re-run with Full mode before making content strategy or investment decisions.*
...
2. Based on the above, propose competitors that occupy the same or adjacent positioning. **Full mode: propose 5 competitors. Demo mode: propose 3 competitors.** For each, provide:
...
**Goal:** Generate test prompts grounded in real demand signals — not from your own knowledge.

**HARD RULE (applies to both modes): Do not generate any prompt from your own knowledge or training data. Every prompt must be traceable to a specific source actually visited during this step.** If you skip the research and generate prompts that "sound reasonable," the entire audit is compromised.

---

#### Full mode: 15 prompts from 3 independent sources
...
**Self-check before proceeding (Full mode):** Look at your 15 prompts. Does every single one have a real source in the "Source" column? Sources must be one of: competitor blog/FAQ (with URL), Reddit thread (with subreddit), or Google PAA (with query used). If any prompt has no source — you made it up. Delete it and replace it with one grounded in evidence.

**Deliverable (Full mode):** Table of 15 prompts with sources, ready for Step 3 testing.

---

#### Demo mode: 5 prompts from competitor websites + web search

**Method: Single research pass — no separate sub-agents needed.**

1. Visit the blog / insights / resources / FAQ page of each of the 3 competitors confirmed in Step 0.
2. Extract 3–5 actual article titles, FAQ questions, or webinar topics from each competitor. Record competitor name, page URL, and the extracted titles.
3. Run 2–3 broad web searches relevant to the target firm's practice areas to surface additional question patterns (e.g., common pain points, search-driven user intent). Record the queries used and the surfaced topics.
4. Consolidate findings:
   - Deduplicate similar questions
   - Select **5 prompts** that cover the target firm's major practice areas
   - Rewrite each prompt in natural user language — conversational, not corporate
   - Tag each prompt: FIND / UNDERSTAND / COMPARE / RISK
   - Aim for at least 1 of each intent type if possible (the 5 prompts won't cover all 4 intent types perfectly — that's expected for a Demo)
   - Record the source for each prompt (competitor URL or search query)

**Output format (Demo mode):**

| # | Prompt | Practice Area | Intent | Source |
|---|--------|---------------|--------|--------|
| 1 | "[user question]" | [practice area] | FIND/UNDERSTAND/COMPARE/RISK | Competitor URL or web search query |

**Self-check before proceeding (Demo mode):** Every prompt must have a real source. If a prompt has no source — you made it up. Delete it and replace it with one grounded in evidence.

**Deliverable (Demo mode):** Table of 5 prompts with sources, ready for Step 3 testing.
...
For each prompt from Step 2 (**15 prompts in Full mode, 5 prompts in Demo mode**):
...
| Metric | Result |
|--------|--------|
| Target firm mentioned | X out of N prompts (X%) where N = 15 (Full) or 5 (Demo) |
| Positive mentions | X |
| Neutral mentions | X |
| Negative mentions | X |
| Top competitor mentions | [Firm A: Y/N, Firm B: Z/N] |
